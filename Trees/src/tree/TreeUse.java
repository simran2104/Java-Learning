...
public class TreeUse {
	...
	public static int largestDataOfNode(TreeNode<Integer> root) {
		...
		for(int i=0; i<root.children.size(); i++) {
			int temp = largestDataOfNode(root.children.get(i));
	...
	public static void printNodeWithDepthK(TreeNode<Integer> root, int k) {
		if(k==-1 || root == null) {
			return;
		}
		if(k==0) {
		...
		for(int i=0; i<root.children.size(); i++) {
			printNodeWithDepthK(root.children.get(i), k-1);
		}
	}
	...
	public static void printPreOrderTraversal(TreeNode<Integer> root) {
		...
		5
		for(int i=0; i< root.children.size(); i++) {
			printPreOrderTraversal(root.children.get(i));
		}
	}
...
//		System.out.println("Enter the Depth of Node you want to print: ");
//		int depth = sc.nextInt();
//		System.out.println("Nodes whose depth is " + depth + " are: ");
//		printNodeWithDepthK(root, depth);
